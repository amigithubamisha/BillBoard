...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        const transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: 'amisha.nanda2000@gmail.com', // Your email
                pass: 'abcd efgh' // Your Gmail App Password (or regular password if not using two-factor authentication)
            },
            tls: {
        ...
        const mailOptions = {
            from: 'amishananda2000@gmail.com',
            to: user.email,
...
router.get("/profile", requireLogin, async (req, res) => {
  ...
  router.get("/userPosts/:userId",requireLogin, async (req, res) => {
    try {
      const userId = req.params.userId;
  
      // Check if the user exists
      const user = await USER.findById(userId);  // Fix the case here
      if (!user) {
